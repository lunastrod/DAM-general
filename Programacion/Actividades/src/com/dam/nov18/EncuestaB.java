...
 Que tenga un método que reciba un síntoma como parámetro e indique con un
booleano si se encuentra entre los síntomas que padeció o no.
Ejemplo: Si recibiera Cansancio como parámetro debería devolver un true.
Si recibiera Diarrea como parámetro debería devolver un false.
...
public class EncuestaB {
    ...
    public EncuestaB(int edad, boolean covid, String sintomas, int gravedad) {
        this.edad = edad;
        this.covid = covid;
    ...
    public boolean sintomaPadecido(String sintoma) {
        String[] sintomasArray = sintomas.split(",");
        boolean encontrado=false;
        for(int i=0;!encontrado&&i<sintomasArray.length;i++){
            sintomasArray[i]=sintomasArray[i].toLowerCase().trim();
            if(sintomasArray[i].equals(sintoma)){
                encontrado=true;
            }
        }
        return encontrado;
